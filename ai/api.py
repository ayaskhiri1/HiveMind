...
def list_models():
    ...
    try:
        ...
        # OPTION 1: Utiliser requests pour plus de contrôle
        import requests
        response = requests.get("http://127.0.0.1:11434/api/tags", timeout=10)
        
        if response.status_code == 200:
            data = response.json()
            logger.debug(f"Réponse brute Ollama: {data}")
            
            # Extraction des noms de modèles
            models = []
            if 'models' in data:
                for model in data['models']:
                    if 'name' in model:
                        models.append(model['name'])
                    elif 'model' in model:  # Certaines versions utilisent 'model'
                        models.append(model['model'])
            
            logger.info(f"Modèles trouvés via API directe: {models}")
        ...
        logger.error("Connexion à Ollama impossible")
        return jsonify({
            "success": False,
        ...
        logger.error(f"Erreur lors de la liste des modèles: {str(e)}", exc_info=True)
        
        # Fallback: vérifier si au moins le modèle par défaut est disponible
        try:
            # Test simple avec le modèle par défaut
            test_result = detect_anomaly("Test de connexion", DEFAULT_MODEL)
            models = [DEFAULT_MODEL] if test_result else []
            ...
            return jsonify({
                ...
                "data": {
                    ...
                    "total_models": len(models),
                    "recommended_models": ["llama3:latest", "mistral", "gemma:2b"],
                    "ollama_status": "degraded" if models else "disconnected",
                    "note": "Liste partielle - erreur de parsing"
                },
                "warning": f"Liste partielle: {str(e)}"
            ...
            return jsonify({
                ...
                "details": str(e),
                "suggestion": "1. Vérifiez qu'Ollama est installé\n2. Exécutez 'ollama serve'\n3. Téléchargez un modèle: 'ollama pull llama3:latest'"
            }), 500
